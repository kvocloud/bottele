...
SS_ADMIN_ACCOUNT = "kvocloud@gmail.com/Tungvu@23" 
TARGET_CHAT_ID = 7365030386 
...
################################################################################
# PHẦN A: HÀM API CORE CHO LẤY TOKEN (TÁCH BIỆT THEO HỆ THỐNG)
################################################################################
...
# Xử lý lệnh /get (Manual check hết hạn)
async def get_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Logic của get_handler để kiểm tra thủ công (nếu có)
...
async def scheduled_get(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not token_kvocloud:
        await context.bot.send_message(TARGET_CHAT_ID, "⚠️ LỖI: Không lấy được KVO Cloud Token trong quá trình quét hết hạn.")
        return

    # Toàn bộ logic API call cũ để lấy và nhóm VPS/Proxy hết hạn (tool.py)
    
    try:
        # --- PHẦN CODE NÀY PHẢI ĐƯỢC DÁN HOÀN CHỈNH TỪ tool.py NẾU CÓ ---
        # Do không có code get_expired_data, tôi tạm thời mô phỏng logic
        
        today = datetime.now(VIETNAM).strftime("%d/%m/%Y")
        
        grouped_vps = {} # Kết quả thực tế
        grouped_pro = {} # Kết quả thực tế
        het_han_vps = "Chưa rõ"
        het_han_pro = "Chưa rõ"
        
        if not grouped_vps and not grouped_pro: 
            # Giả định: Nếu API call thực tế không có data hết hạn, hàm sẽ return.
            return

        msg = f"Hôm nay: {today}\n🔑API KEY: {token_kvocloud}\n"
        ...
        await context.bot.send_message(TARGET_CHAT_ID, f"⚠️ Lỗi khi chạy scheduled_get (KVOCloud): {e}")
...
def main() -> None:
    ...
    if BOT_TOKEN == "8450329079:AAEsVOZz_rBU9iusTmZaiKcaIKu11MrU8vY" or TARGET_CHAT_ID == 7365030386 or SS_ADMIN_ACCOUNT == "kvocloud@gmail.com/Tungvu@23":
    ...
    print("Thiết lập Job báo cáo hết hạn KVOCloud (8:30 & 20:30)...")
    job_queue.run_daily(
        scheduled_get, 
    ...
    print("Thiết lập Job Giám sát Client Smart Server (12:00)...")
    job_queue.run_daily(
        monitor_new_clients, 
        time=time(hour=12, minute=0, tzinfo=VIETNAM), 
        days=(0, 1, 2, 3, 4, 5, 6),
